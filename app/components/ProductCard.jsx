...
export default function ProductCard({ p = {}, onAddToCart, isFeatured = false, isScrollRow = false }) {
  ...
  const mainImage = p.image || p.image_url || PLACEHOLDER_IMG;
  const hoverImage = p.image2 || mainImage;
  ...
  const handleAddToCartClick = async (e) => {
    ...
    // 1. Simpan ke localStorage (untuk respon instan UI)
    const existingCart = JSON.parse(localStorage.getItem("cartItems")) || [];
    const updatedCart = [...existingCart, p];
    localStorage.setItem("cartItems", JSON.stringify(updatedCart));

    // 2. Simpan permanen ke tabel cart_items Supabase beserta user_id-nya
    const { error } = await supabase
      .from("cart_items")
      .insert([
        {
          user_id: user.id, // Ditambahkan agar tidak melanggar not-null constraint
    ...
    // 3. Panggil props onAddToCart jika ada
    if (onAddToCart) {
      onAddToCart(p);
    }
  };
