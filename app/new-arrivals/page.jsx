...
export default async function NewArrivalsPage() {
  ...
  // Ambil data produk dari Supabase, diurutkan dari yang terbaru (berdasarkan id/waktu)
  ...
  if (error) {
    console.error("Gagal mengambil produk New Arrivals:", error.message);
  }

  const allProducts = products || [];

  // Pemetaan properti agar sesuai dengan ProductCard.jsx
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="flex-grow pb-16">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 text-xs sm:text-sm text-gray-500">
          <Link href="/" className="hover:underline">Home</Link> / <span className="text-gray-800 font-medium">New Arrivals</span>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <CategoryFilterSidebar 
            ...
            subCategories={["All Products", "Clothing", "Shoes", "Home & Living"]} 
          />

          <div className="lg:col-span-3">
            <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-gray-100 shadow-sm mb-6">
              <h1 className="text-lg font-bold text-gray-900">New Arrivals ({formattedProducts.length} Produk)</h1>
            ...
            {formattedProducts.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                {formattedProducts.map((product) => (
                  <ProductCard key={product.id} p={product} isFeatured={true} />
                ))}
              </div>
            ) : (
              <div className="bg-white rounded-xl p-8 text-center border border-gray-100 text-gray-500 text-sm">
                Belum ada produk New Arrivals di database Supabase.
              </div>
            )}
          </div>
